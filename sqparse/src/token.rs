...
define_terminals! {
    identifiers {
        ...
        // fixme: this is a hack for some places that define constructors out-of-line
        Constructor => "constructor"                        if not(Flavor::SquirrelRespawn),
...
struct NotFlavor(Flavor);

impl PartialEq<Flavor> for NotFlavor {
    fn eq(&self, other: &Flavor) -> bool {
        self.0 != *other
    }
}

fn not(flavor: Flavor) -> NotFlavor {
    NotFlavor(flavor)
}
